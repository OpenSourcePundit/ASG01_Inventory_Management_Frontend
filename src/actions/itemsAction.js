...
const fetchAllItems = () => async (dispatch) => {
  ...
  try {
    const response = await fetch(
      "https://inventory-management-pu1d.onrender.com/api/v1/item/all",
    );
...
const addItem = (item) => async (dispatch) => {
  ...
  try {
    const response = await axios({
      method: "POST",
      url: "https://inventory-management-pu1d.onrender.com/api/v1/item/new",
      data: item,
    });
...
const deleteItem = (id) => async (dispatch) => {
  ...
  try {
    const response = await axios({
      method: "DELETE",
      url: `https://inventory-management-pu1d.onrender.com/api/v1/item/delete/${id}`,
    });
...
const updateItem = (id, item) => async (dispatch) => {
  ...
  try {
    const response = await axios({
      method: "PUT",
      url: `https://inventory-management-pu1d.onrender.com/api/v1/item/update/${id}`,
      data: item,
    });
